...
//pkg.squire.eu/npm/:_authToken=<YOUR_TOKEN>" > .npmrc
npm install
npm run dev
...
A token is required to authenticate your requests to the Squire API. You can obtain a token by making a POST request to the token endpoint with your API key and user details.
...
Once you have the token, you can pass it to the Squire SDK to authenticate your requests.

In a real life application, you would request the token on your backend server and not expose your API key in the frontend code.
